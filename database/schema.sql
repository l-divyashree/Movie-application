...
CREATE TABLE movies (
    ...
    release_date DATE,
    poster_url VARCHAR(500), -- TODO: Replace with real URLs
    trailer_url VARCHAR(500), -- TODO: Replace with real URLs
...
CREATE TABLE venues (
    ...
    phone_number VARCHAR(20),
    facilities JSON, -- Parking, Food Court, etc.
    is_active BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE shows (
    ...
    screen_name VARCHAR(50), -- Screen 1, Screen 2, etc.
...
CREATE TABLE seats (
    ...
    is_blocked BOOLEAN DEFAULT FALSE, -- Temporarily blocked during booking
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE bookings (
    ...
    booking_status VARCHAR(20) DEFAULT 'CONFIRMED', -- RESERVED, CONFIRMED, CANCELLED
...
-- Events table (Special events, premieres, etc.)
CREATE TABLE events (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    title VARCHAR(255) NOT NULL,
    description TEXT,
    ...
    event_time TIME,
    venue_id BIGINT,
    category VARCHAR(100), -- PREMIERE, SPECIAL_SCREENING, etc.
    price DECIMAL(10, 2),
    poster_url VARCHAR(500), -- TODO: Replace with real URLs
    is_active BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
-- Sports table (Sports events and matches)
CREATE TABLE sports (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    title VARCHAR(255) NOT NULL,
    description TEXT,
    sport_type VARCHAR(100), -- CRICKET, FOOTBALL, BASKETBALL, etc.
    event_date DATE,
    event_time TIME,
    venue_id BIGINT,
...
('admin', 'admin@moviebooking.com', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'Admin', 'User', '+91-9999999999');

-- Regular user (password: user123)
INSERT INTO users (username, email, password, first_name, last_name, phone_number) VALUES 
('john_doe', 'john@example.com', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'John', 'Doe', '+91-9876543210');
...
(1, 1, CURDATE(), '22:00:00', 'Screen 1', 300.00, 150, 150),

-- KGF Chapter 2 shows
(2, 2, CURDATE(), '11:00:00', 'Screen 2', 280.00, 120, 120),
(2, 2, CURDATE(), '15:30:00', 'Screen 2', 320.00, 120, 120),
(2, 2, CURDATE(), '19:30:00', 'Screen 2', 380.00, 120, 120),

-- RRR shows
(3, 3, CURDATE(), '12:00:00', 'Screen 3', 300.00, 100, 100),
(3, 3, CURDATE(), '16:30:00', 'Screen 3', 350.00, 100, 100),
(3, 3, CURDATE(), '21:00:00', 'Screen 3', 400.00, 100, 100);

-- Generate seats for shows (This is a sample - in practice you'd want a more efficient approach)
-- For show 1 (Avengers Endgame - Screen 1)
INSERT INTO seats (show_id, seat_row, seat_number, seat_type, price, is_available) 
SELECT 1, seat_row, seat_number, 
    CASE 
        WHEN seat_row IN ('A', 'B') THEN 'PREMIUM'
        WHEN seat_row IN ('H', 'I', 'J') THEN 'VIP'
        ELSE 'REGULAR'
    END as seat_type,
    CASE 
        WHEN seat_row IN ('A', 'B') THEN 250.00
        WHEN seat_row IN ('H', 'I', 'J') THEN 350.00
        ELSE 200.00
    END as price,
    TRUE as is_available
FROM (
    SELECT CHAR(65 + units.i + tens.i * 10) as seat_row, ones.i + 1 as seat_number
    ...
    CROSS JOIN (SELECT 0 as i) units
    WHERE ones.i + 1 <= 10 AND CHAR(65 + units.i + tens.i * 10) <= 'J'
) seat_grid;
